...
const renderItems = (offer) => {
  ...
  const objectTypes = new Map([['palace', 'Дворец'], ['house', 'Дом'], ['bungalow', 'Бунгало'], ['flat', 'Квартира'], ['hotel', 'Отель']]);

  for (const obj of objectTypes) {
    if (obj[0] === offer.type) {
      popupCard.querySelector('.popup__type').textContent = obj[1];
    }
  }
};
